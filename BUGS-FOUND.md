...
### BUG-054 — GSTR-1 JSON download wrapped in extra { gstr1: ... } envelope (Critical/GST-filing) — FIXED
- **File**: `src/components/reports/Gstr1Report.tsx:67` (handleDownloadJSON)
- **Severity**: Critical (the downloaded JSON file is rejected by GST portal validators)
- **Found**: 2026-07-19, user live testing with a GSTR-1 JSON schema validator
- **Description**: The download handler did `JSON.stringify({ gstr1: data.gstr1 }, null, 2)` — wrapping the entire GSTR-1 data inside an outer `{ "gstr1": {…} }` object. The GST portal's JSON schema requires `gstin` and `fp` (along with every other top-level field) to sit at the ROOT of the file, not nested inside a `"gstr1"` wrapper. Portal validators reject the wrapped file with "Missing required key(s): gstin, fp" — they're not missing from the data, just nested one level too deep.
- **Root cause**: The API response is correctly shaped as `{ period, gstr1, summary, snapshot, shop }` (the component needs period/summary for display). But the download handler wrapped `data.gstr1` in `{ gstr1: ... }` before stringifying, instead of stringifying `data.gstr1` directly. Likely a copy-paste from the API response shape — the developer assumed the download should mirror the API envelope, but the portal expects the raw GSTR-1 object.
- **Fix applied**: 2026-07-19 (V26 Batch 6). Changed `JSON.stringify({ gstr1: data.gstr1 }, null, 2)` → `JSON.stringify(data.gstr1, null, 2)`. The downloaded file now starts with `{ "gstin": "...", "fp": "072026", "gt": …, "cur_gt": …, "b2b": […] }` directly — no wrapper. Added explanatory comment documenting the portal schema requirement.
...
### BUG-062 — CLOSED 2026-08-14. The note lookup only worked in the rarer case.
- **The earlier fix was half a fix, and its comment read as a whole one.** `resolveNoteClassification` did look up the original invoice — but with `txns.find(...)`, and `txns` is only the filing month's transactions. **A credit note is almost always issued in a LATER month than the invoice it cancels**, so the original was usually absent, the lookup silently failed, and it fell back to the note's own values — exactly the behaviour BUG-062 was raised about. It worked only when note and original happened to land in the same month.
- **Why it matters**: the classification decides where the note is filed. CDNUR (Table 9B) entries are declared `typ: 'B2CL'`, which the portal cross-validates against an inter-state place of supply. A note misfiled into CDNUR is rejected on upload; one wrongly netted into B2CS understates the reduction. Either way the return is wrong, and nothing in the app would say so.
- **Fix**: the route now fetches the originals for exactly those note ids whose invoice is outside the period — by id, unbounded by date, `userId`-scoped so one shop can never be classified by another's invoice — and passes them to the builder. For a month with no cross-period notes the extra query does not run at all.
- **Guard**: `src/__tests__/lib/gstr1-note-classified-by-its-original.test.ts` — 11 tests. The load-bearing pair asserts **both directions**: without the lookup the note is misfiled (the bug, demonstrated), and with it the note reaches CDNUR. Also covers the reverse error — a note that *looks* like B2CL on its own values but whose original was small must be netted into B2CS instead. Break-verified: making the builder ignore the lookup fails 5.
- **The guard caught my own incomplete wiring.** My scripted edit reported success while matching nothing (line endings again), so only the SECOND of the two `buildGstr1` call sites got the lookup — the main GSTR-1 endpoint was left guessing. The call-site test found it. It balances parentheses rather than reading a fixed window, because this repo has been bitten three times by guards that measured nearby text instead of structure.
- **Still worth doing on the first real portal upload**: confirm the portal accepts a CDNUR entry classified this way. The rule is now applied from the original invoice, which is what the GST rules describe, but only an actual upload proves the portal agrees.
- **Status**: RESOLVED (code); portal confirmation remains a Phase 1b checklist item.
